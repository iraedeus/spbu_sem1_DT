...
def print_rules():
    print('The computer makes a four-digit number with non-repeating digits. You need to guess it by entering your number.\n'
...
def check_digits(computer_num, user_num):
    ...
    for i in range(len(computer_num)):
        if user_num[i] == computer_num[i]:
            ...
            count_of_cows += 1
    return paint_in_color(f'{count_of_bulls} bulls\n', 'green') + paint_in_color(f'{count_of_cows} cows', 'yellow')
