...
def request_to_user_action(object):
    if object == 'vector':
        input_action_vector = input("1) Find scalar product of vectors \n"
                                    "2) Find length of vector \n"
                                    "3) Find angle between vectors: \n")
        return input_action_vector
    elif object == 'matrix':
        input_action_matrix = input('1) Find sum of matrices \n'
                                    '2) Find product of matrices \n'
                                    '3) Transpose a matrix: \n')
        return input_action_matrix
    elif object == 'choice':
        print(paint_text('Enter the number to perform the action: \n'
                         '1) Perform actions on vectors\n'
                         '2) Perform actions on matrices\n'
                         '3) Exit the program', 'white'))
    return None


def request_to_user_vector(sequence):
    if sequence == 'first':
        return format_vector(input('Enter your first vector in format: (x1,x2,x3....xn), xn is a number\n'))
    elif sequence == 'second':
        return format_vector(input(paint_text('Enter your second vector in format: (y1,y2,y3....yn), yn is a number\n', 'white')))


def request_to_user_matrix(sequence):
    if sequence == 'first':
        return format_matrix(input('Enter you first matrix in format: '
                                   '(x11,x12...x1n)-(x21,x22...x2n)...(...) x1n,x2n are numbers\n'))
    elif sequence == 'second':
        return format_matrix(
            input(paint_text('Enter you second matrix in format: (x11,x12...x1n)-(x21,x22...x2n)...(...) '
                             'x1n,x2n are numbers\n', 'white')))
...
def find_angle_between_vectors(vector_1, vector_2):
    if not check_vectors_dimensions(vector_1, vector_2):
        return []
    return acos(find_scalar_product(vector_1, vector_2) / (
            find_length_of_vector(vector_1) * find_length_of_vector(vector_2))) * 180 / pi


def find_sum_of_matrices(matrix_1, matrix_2):
    if not correctness_of_matrix_check(matrix_1) or not correctness_of_matrix_check(matrix_2):
        return []
    if (len(matrix_1[0]) != len(matrix_2[0])) or (len(matrix_1) != len(matrix_2)):
        print(paint_text('You cannot sum these matrices', 'red'))
        return []
    return [[matrix_1[i][j] + matrix_2[i][j] for j in range(len(matrix_2[0]))] for i in range(len(matrix_1))]
...
def action_with_vector():
    if action_from_user == '1':
        ...
        print(paint_text('Scalar product of your vectors is: ' + str(find_scalar_product(vector_1_input, vector_2_input)), 'yellow'))
    elif action_from_user == '2':
        vector_input = format_vector(input('Enter your vector in format: (x1,x2, 3....xn), xn is a number\n'))
        print(paint_text('Length of your vector is: ' + str(find_length_of_vector(vector_input)), 'yellow'))
    elif action_from_user == '3':
        vector_1_input = request_to_user_vector('first')
        vector_2_input = request_to_user_vector('second')
        print(paint_text(
            'Angle between your vectors is: ' + str(find_angle_between_vectors(vector_1_input, vector_2_input)),
            'yellow'))
    else:
        print(paint_text('You have chosen an incorrect action, please try again', 'red'))
...
def action_with_matrices():
    if action_from_user == '1':
        ...
        matrix_input = format_matrix(input('Enter you matrix in format:'
                                           '(x11,x12...x1n)-(x21,x22...x2n)...(...) x1n,x2n are numbers\n'))
