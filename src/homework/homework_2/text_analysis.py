...
def find_counts_each_word(path):
    ...
    for word in reading_file(path):
        if word in counts_each_word:
            ...
            counts_each_word[word] = 1
    return sorted(
        counts_each_word.items(), key=lambda x: x[1]  # [('Hello', 1), ('world', 1)]
    )
...
if __name__ == "__main__":
    txt_file_path = find_file_path(
        input("Enter name of your file in format XXX.txt: "), "/"
    )
    csv_file_path = find_file_path(
        input("Enter name of your file in format XXX.csv: "), "/"
    )

    if txt_file_path is None or csv_file_path is None:
        print(
            "We cannot write data from txt file to csv file because one of them doesn't exist"
        )
    else:
